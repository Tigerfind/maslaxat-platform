...
Сейчас схема применяется через `sequelize.sync({ alter: true })` — удобно в разработке,
но в проде рискованно. Перед первым продакшен-запуском:

1. Поднять PostgreSQL, создать базу и пользователя.
2. Первый старт с `sync({ alter: true })` создаст все таблицы.
3. **Для дальнейших изменений схемы** перейти на миграции (sequelize-cli) вместо `alter`,
   чтобы не терять данные. (Задача в BACKLOG: перевести на миграции + удалить мёртвые
   столбцы `Consultation.rating/review`.)
...
- Перейти с `sync({alter})` на миграции; удалить мёртвые столбцы `Consultation.rating/review`.
