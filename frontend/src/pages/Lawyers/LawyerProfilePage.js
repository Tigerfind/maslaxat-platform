      ...
      <div className="lpv2-wrap">
        ...
        <div className="lpv2-layout">
          ...
          <aside className="lpv2-card lpv2-book-card" aria-label={t('lawyerProfileV2.bookingCard')}>
            ...
            {lawyer.consultationDurations.length > 0 && <div><h3>{t('lawyerProfileV2.duration')}</h3><div className="lpv2-duration-list">{lawyer.consultationDurations.map((duration) => { const exactPrice = Number(lawyer.durationPrices?.[String(duration)]); const displayedPrice = Number.isSafeInteger(exactPrice) ? exactPrice : Math.round(lawyer.priceFrom * duration / 60); return <span key={duration}><strong>{duration} {t('lawyerProfileV2.min')}</strong>{lawyer.priceFrom > 0 && <small>{displayedPrice.toLocaleString(LOCALES[language])} {t('lawyerProfile.sum')}</small>}</span>; })}</div></div>}
            {lawyer.consultationFormats.length > 0 && <div><h3>{t('lawyerProfileV2.formats')}</h3><div className="lpv2-format-list">{lawyer.consultationFormats.map((format) => { const Icon = FORMAT_ICONS[format] || ChatBubbleOutline; return <span key={format}><Icon />{formatName(format)}</span>; })}</div></div>}
