...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    // Sync-era databases used both acronym spellings across Sequelize versions.
    for (const table of ['ai_messages', 'a_i_messages'].filter((name) => tables.includes(name))) {
      const aiColumns = await queryInterface.describeTable(table);
      ...
      if (!aiColumns.fallback) {
        await queryInterface.addColumn(table, 'fallback', {
  ...
  async down(queryInterface) {
    ...
    for (const table of ['ai_messages', 'a_i_messages'].filter((name) => tables.includes(name))) {
      const aiColumns = await queryInterface.describeTable(table);
      if (aiColumns.fallback) await queryInterface.removeColumn(table, 'fallback');
      if (aiColumns.sources) await queryInterface.removeColumn(table, 'sources');
    }
