...
test('юрист сохраняет итог, клиент видит его в деталях', async () => {
  ...
  const save = await request(app).patch(`/api/consultations/${consultation.id}/summary`)
    .set('Authorization', `Bearer ${tokenFor(lawyer)}`).set('X-Maslaxat-Mode', 'lawyer').send({ summary });
...
test('завершение из кабинета юриста требует итог и сохраняет его вместе с completed', async () => {
  ...
  const auth = `Bearer ${tokenFor(lawyer, 'mfa')}`;
  expect((await request(app).post(`/api/lawyer/consultations/${consultation.id}/end`)
    .set('Authorization', auth).set('X-Maslaxat-Mode', 'lawyer').send({ notes: '' })).status).toBe(400);
  ...
  const ended = await request(app).post(`/api/lawyer/consultations/${consultation.id}/end`)
    .set('Authorization', auth).set('X-Maslaxat-Mode', 'lawyer').send({ notes: summary });
