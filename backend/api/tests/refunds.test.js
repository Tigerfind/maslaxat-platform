...
const { Consultation, Payment, LawyerProfile, FinancialTransaction } = models;
...
test('локальная отмена только запрашивает возврат, Payme подтверждает его отдельно', async () => {
  ...
  const consultation = await Consultation.create({
    ...
    commissionRateBps: 0, grossAmountTiyin: 18000000, lawyerNetAmountTiyin: 18000000,
  });
  const payment = await Payment.create({
    userId: client.id, consultationId: consultation.id, amount: 180000,
    amountTiyin: 18000000, refundedAmountTiyin: 0, purpose: 'consultation',
  ...
  expect(Number(lp.pendingBalance)).toBe(180000);
  ...
  expect(await FinancialTransaction.count({ where: { paymentId: payment.id } })).toBe(1);
});
...
test('завершение не воскрешает отменённую консультацию и не выпускает escrow', async () => {
  ...
  const consultation = await Consultation.create({
    ...
    commissionRateBps: 0, grossAmountTiyin: 10000000, lawyerNetAmountTiyin: 10000000,
  });
  await Payment.create({
    userId: client.id, consultationId: consultation.id, amount: 100000,
    amountTiyin: 10000000, refundedAmountTiyin: 0, purpose: 'consultation',
