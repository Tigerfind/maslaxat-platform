...
describe('#3 free-consultation — нет double-spend под конкуренцией', () => {
  const book = (token, lawyerId, body) =>
    request(app).post(`/api/client/lawyers/${lawyerId}/book`).set('Authorization', `Bearer ${token}`).send({ consultationType: 'chat', acceptedTerms: true, legalVersion: '2026-08-13', ...body });
