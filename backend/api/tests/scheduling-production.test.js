...
test.each([30, 60, 90])('длительность %i хранится числом и целиком входит в расписание', async (duration) => {
  ...
  const response = await request(app).post(`/api/lawyers/${lawyer.id}/book`).set('Authorization', `Bearer ${tokenFor(client)}`).send({
...
test('90 минут перед закрытием расписания отклоняются', async () => {
  ...
  const response = await request(app).post(`/api/lawyers/${lawyer.id}/book`).set('Authorization', `Bearer ${tokenFor(client)}`).send({ question: 'Late', consultationType: 'video', duration: 90, preferredDate: nextDay(1).toISODate(), preferredTime: '17:00', acceptedTerms: true, legalVersion: '2026-08-13' });
...
test('reschedule повторно проверяет статус под row lock', async () => {
  ...
  const [cancel, move] = await Promise.all([
    request(app).post(`/api/consultations/${consultation.id}/cancel`).set('Authorization', `Bearer ${tokenFor(client)}`).send({ reason: 'cancel' }),
    request(app).patch(`/api/consultations/${consultation.id}/reschedule`).set('Authorization', `Bearer ${tokenFor(client)}`).send({ preferredDate: start.toISODate(), preferredTime: '11:00' }),
  ]);
