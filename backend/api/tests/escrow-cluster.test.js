...
async function seedPaid({ status = 'accepted', price = 200000 } = {}) {
  ...
  const cons = await Consultation.create({
    clientId: client.id, lawyerId: lawyer.id, question: 'q', status, price,
    commissionRateBps: 0, grossAmountTiyin: price * 100, lawyerNetAmountTiyin: price * 100,
  });
  const pay = await Payment.create({
    userId: client.id, consultationId: cons.id, purpose: 'consultation', amount: price, amountTiyin: price * 100,
...
describe('reject-after-pay requests provider refund', () => {
  test('lawyer rejects a paid pending consultation → payment refund_pending, reserve unchanged', async () => {
    ...
    expect(Number(lawyerProfile.pendingBalance)).toBe(price); // ждём подтверждения провайдера
