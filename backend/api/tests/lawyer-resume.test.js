...
test('draft сохраняет структурированное резюме и owner preview после refresh', async () => {
  ...
  const save = await request(app).patch('/api/lawyer/profile/draft').set('Authorization', `Bearer ${token}`).set('X-Maslaxat-Mode', 'lawyer').send(resumePayload);
  ...
  const preview = await request(app).get('/api/lawyer/profile/preview').set('Authorization', `Bearer ${token}`).set('X-Maslaxat-Mode', 'lawyer');
...
test('submit → approve публикует резюме и пишет status history', async () => {
  ...
  await user.update({ avatar: '/uploads/avatar-test.png', twoFactorEnabled: true, twoFactorSecret: 'TESTSECRET' });
  const token = tokenFor(user, 'mfa');
  await request(app).patch('/api/lawyer/profile/draft').set('Authorization', `Bearer ${token}`).set('X-Maslaxat-Mode', 'lawyer').send(resumePayload);
  await models.LawyerDocument.create({ userId: user.id, type: 'license', name: 'license.pdf', path: '/private/license.pdf' });
  const submit = await request(app).post('/api/lawyer/verification/submit').set('Authorization', `Bearer ${token}`).set('X-Maslaxat-Mode', 'lawyer');
  expect(submit.status).toBe(200);
  expect(submit.body.verificationStatus).toBe('pending_review');
  expect((await request(app).post('/api/lawyer/verification/submit').set('Authorization', `Bearer ${token}`).set('X-Maslaxat-Mode', 'lawyer')).status).toBe(409);
...
test('валидация отклоняет невозможные даты и hostile LinkedIn URL', async () => {
  ...
  const token = tokenFor(user);
  const bad = await request(app).patch('/api/lawyer/profile/draft').set('Authorization', `Bearer ${token}`).set('X-Maslaxat-Mode', 'lawyer').send({
