...
test('повторные и параллельные create возвращают один pending Payment', async () => {
  ...
  const call = () => request(app).post('/api/payments/create').set('Authorization', auth)
    .set('Idempotency-Key', 'payment-resume-create').send({ consultationId: consultation.id });
...
test('повторная simulation идемпотентна и резервирует escrow ровно один раз', async () => {
  ...
  expect(Number(lp.pendingBalance)).toBe(212500);
