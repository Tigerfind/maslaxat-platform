...
test('первое принятие заявки фиксирует acceptedAt, повтор не перезаписывает', async () => {
  ...
  expect((await request(app).post(`/api/lawyer/consultation-requests/${consultation.id}/accept`).set('Authorization', auth).set('X-Maslaxat-Mode', 'lawyer')).status).toBe(200);
