...
Наши юристы по гражданскому праву помогут составить претензию.`,
  ...
  // Ничего не совпало: честно сообщаем, что AI-помощник сейчас недоступен, и ведём
  // к живому юристу — вместо меню категорий, которое имитировало работу ассистента
  // и зацикливало пользователя (реальные ответы даёт только Claude при заданном ключе).
  return {
    reply: `⚠️ **AI-помощник временно недоступен**

Извините, автоматический AI-консультант сейчас не может ответить на ваш вопрос. Чтобы получить помощь со ссылками на законы Республики Узбекистан, запишитесь на консультацию к **живому юристу** через раздел «Юристы» — специалист ответит подробно.`,
    category: null,
  };
};
